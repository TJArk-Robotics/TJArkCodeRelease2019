...
int MofCompiler::myprintf(const char *format, ...)
{
    ...
    return vsnprintf(printBuffer + size, printBufferSize - size, format, args);
}
    ...
    while (thereAreMore)
    {
        if (strcmp(ff->d_name, "extern.mof"))
        {
            ...
            else
            {
                ...
                std::string motionStr(motion);
                actMotionID = SpecialActionIDValue[motionStr];
                    ...
                    while (*t)
                    {
                        ...
                        if (u >= t)
                        {
                            *u = 0;
                            ...
                            char sval[Joints::NumOfJoint + 3][256];                                                                     // joints + interpolate + duration + bad argument
                            ...
                            else if (!strcmp(sval[0], "transition"))
                            {
                                ...
                                else
                                {
                                    ...
                                    std::string svalStr(sval[1]);
                                    found = SpecialActionIDValue[svalStr] >= 0;
                                }
                            ...
                            else if (!strcmp(sval[0], "stiffness"))
                            {
                                ...
                                if (c == Joints::NumOfJoint + 2)
                                {
                                    ...
                                    strcat(temp, " ");
                                    strcat(temp, sval[Joints::NumOfJoint + 1]);
                            ...
                            else if (c == Joints::NumOfJoint + 2)
                            {
                                ...
                                for (int j = 0; j < Joints::NumOfJoint; ++j)
                                {
                                    if (!strcmp(sval[j], "*"))
                                ...
                                if (sscanf(sval[Joints::NumOfJoint], "%i", &val) == 1 && (val >= 0 || val <= 3))
                                {
                                    strcat(temp, " ");
                                    strcat(temp, sval[Joints::NumOfJoint]);
                                }
                                else
                                {
                                ...
                                if (sscanf(sval[Joints::NumOfJoint + 1], "%i", &val) == 1 && val > 0)
                                {
                                    strcat(temp, " ");
                                    strcat(temp, sval[Joints::NumOfJoint + 1]);
                                }
                ...
                if (u >= t)
                {
                    *u = 0;
                    ...
                    else if (!strcmp(sval[0], "transition") && c == 4)
                    {
                        ...
                        {
                            int i;
                            for (i = 0; i < SpecialActionRequest::NumOfSpecialActionID; ++i)
                            {
                                if (!strcmp(SpecialActionIDName[SpecialActionRequest::SpecialActionID(i)].c_str(), sval[1]))
                                {
                                    found = true;
                                    break;
                                }
    ...
    for (int i = 0; i < SpecialActionRequest::NumOfSpecialActionID; ++i)
        if (jumpTable[i] == -1)
        {
            printf("%s/Config/mof/extern.mof(1): error: no motion net entry defined for special action %s\n",
                   File::getBHDir(), SpecialActionIDName[SpecialActionRequest::SpecialActionID(i)].c_str());
            return false;
        }
    return true;
}
...
bool MofCompiler::compileMofs(char *buffer, size_t size, std::vector<float> &motionData)
{
    ...
    for (int i = 1; i < numOfLabels; ++i)
    {
        if (!strcmp(label_motion[i], label_motion[i - 1]))
            continue;
        bool found = false;
        for (int j = 0; j < SpecialActionRequest::NumOfSpecialActionID; ++j)
            if (!strcmp(label_motion[i], SpecialActionIDName[SpecialActionRequest::SpecialActionID(j)].c_str()))
            {
                found = true;
                break;
            }
