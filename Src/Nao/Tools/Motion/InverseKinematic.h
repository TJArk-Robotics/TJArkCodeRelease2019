...
/**
   ...
   * @return Whether the target position was reachable or not (if the given target position is not reachable the computation proceeds using the closest reachable position near the target)
   */
bool calcBalancedLegJoints(const Pose3f &positionLeft, const Pose3f &positionRight, const Quaternionf &bodyRotation,
                           JointAngles &jointAngles, const RobotDimensions &theRobotDimensions, const MassCalibration &theMassCalibration, const JointLimits &theJointLimits, float ratio) WARN_UNUSED_RESULT;
