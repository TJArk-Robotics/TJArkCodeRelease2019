...
public:
   ...
   */
    RingBufferWithSum(std::size_t capacity = n) : RingBuffer<T, n>(capacity),
                                                  zero()
    {
   ...
   *                 the second template parameter is used as default capacity.
   */
    RingBufferWithSum(const T &zero, std::size_t capacity = n) : RingBuffer<T, n>(capacity),
                                                                 zero(zero)
    {
        static_assert(!std::is_same<Angle, T>::value &&
                          !std::is_floating_point<T>::value &&
   ...
   */
    RingBufferWithSum(const RingBufferWithSum &other) : RingBuffer<T, n>(other),
                                                        zero(other.zero),
