...
void ArmKeyFrameEngine::updateArm(Arm &arm, ArmKeyFrameEngineOutput &armKeyFrameEngineOutput)
{
    ...
    {
        const ArmKeyFrameMotion &nextMotion = theArmKeyFrameRequest.arms[arm.id].motion == ArmKeyFrameRequest::reverse ? (arm.currentMotion.id != ArmKeyFrameRequest::reverse ? arm.currentMotion.reverse(defaultPos) : allMotions[ArmKeyFrameRequest::useDefault]) : allMotions[theArmKeyFrameRequest.arms[arm.id].motion];
